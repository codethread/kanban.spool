...
- Lifecycle parity: Kanban removal retracts its static entries without clearing stored cards. Peering removal retracts its two local ops and closes its resource; it does not remove the Guild receiver. Guild retains ownership of the process-lifetime `kanban.send.v1` dispatch entry and receiver table, and the accepted receiver-removal policy is process-lifetime no-op.
