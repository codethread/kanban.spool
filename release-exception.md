...
- Breaking change: the consuming core is now `io.millstrand/millstrand`, the workspace marker is `.millstrand` (with `.ms` accepted by the core), and Kanban source/config/tests/docs use `millstrand.api.*`, `millstrand.spools.*`, and `:millstrand/*` identity. The old cross-workspace card transfer surface is absent. Legacy core names are not compatibility aliases.
- Domain identity retained: `ct.spools.kanban`, `codethread/kanban`, and the core `kanban.*` operation and attribute namespaces.
- Consumer cutover: approve the published Kanban spool from `spools.edn`, then activate `ct.spools.kanban` as a runtime module. Do not call legacy installers or resolve old core namespaces.
